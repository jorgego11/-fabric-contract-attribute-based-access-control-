...
class SupplychainContract extends Contract {
    ...
    */
    async queryAllOrders(ctx) {
        ...
        if (userType.localeCompare('producer') == 0) {
           // The 4 lines below shows an example on how to use the identity affiliation for access control.
           // This only makes sense for usertype 'producer' which is the only one with affiliations
            let userAffiliation = await this.getCurrentUserAffiliation(ctx);    //  e.g.       'hf.Affiliation': 'zzfarm.sales',
            userId = userAffiliation.substring(0, 3).toUpperCase();         //   e.g.   userId = ZZF
            userId += userAffiliation.substring(3, 6);                          // e.g.   userId = ZZFarm
            console.info('userId extracted from affiliation is : ' + userId);
        } else { 
            userId = await this.getCurrentUserId(ctx);                    //  this was the old implementation before using affiliation
        }
    ...
    async getOrderHistory(ctx, orderId) {
        ...
        while (true) {
            ...
            if (history.value && history.value.value.toString()) {
                ...
                if (typeof history.value.is_delete !== 'undefined') { // JGO  Added this to bypass Cannot read property \'toString\' of undefined'
    ...
    /**
      ...
      * To be called by application to get the affiliation for a user who is logged in
      *
      * @param {Context} ctx the transaction context
      * Usage:  getCurrentUserAffiliation ()
     */
    async getCurrentUserAffiliation(ctx) {
        
        console.log("About to log ctx.clientIdentity:  ");
        console.log(ctx.clientIdentity);

        return ctx.clientIdentity.getAttributeValue('hf.Affiliation');
    }
