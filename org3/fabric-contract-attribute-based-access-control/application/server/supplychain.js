...
supplychainRouter.route('/register-user').post(function (request, response) {
    try {
        ...
        let affiliationIn = request.body.affiliation;    

        console.log("\n In supplychain.js    userid: " + userId + ", pwd: " + userPwd + ", usertype: " + userType + ", affiliation: " + affiliationIn);
        ...
        getUsernamePassword(request)
            .then(request => {
                ...
                utils.registerUser(userId, userPwd, userType, request.username, affiliationIn).
                    then((result) => {
                        response.status(STATUS_SUCCESS);
